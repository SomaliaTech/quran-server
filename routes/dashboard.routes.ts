...
interface DashboardStats {
  ...
  charts: {
    weeklyActivity: { day: string; posts: number; resolved: number }[];
...
interface RecentActivity {
  id: string;
  type: "post" | "ticket" | "review" | "resolution";
  title: string;
  time: string;
  status?: string;
  user?: string;
...
router.get("/stats", authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const categoryDistribution = categoryStats
      .filter((stat: any) => stat.categories && stat.categories.length > 0)
      .slice(0, 8)
    ...
    const statusBreakdown = statusStats.map((stat: any) => ({
      status: stat.status || "unknown",
      count: stat._count,
      color:
        STATUS_COLORS[stat.status as keyof typeof STATUS_COLORS] || "#95A5A6",
    }));
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const [userPosts, userTickets, userReviews, userActivity] =
        await Promise.all([
...
// ============ HELPER FUNCTIONS ============
function determinePriority(ticket: any): "LOW" | "MEDIUM" | "HIGH" | "URGENT" {
...
function calculateResolutionRate(tickets: any[]): number {
...
function calculateResponseRate(tickets: any[]): number {
